...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="daterules">
    <xsl:choose>
      <xsl:when test="not(@option) or (@option and matches(/options,@option))">
        <xsl:message>daterules for <xsl:value-of select="../name"/> (year : <xsl:value-of
            select="/year"/>)</xsl:message>
        <xsl:apply-templates/>
        <xsl:message>/daterules</xsl:message>
      </xsl:when>
      <xsl:otherwise>
        <xsl:message>daterules option mismatch(option : <xsl:value-of select="@option"/>, options :
            <xsl:value-of select="/options"/>)</xsl:message>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  <!--
  ...
  <xsl:template match="this-date">
    <!-- INPUT $date : yyyy-mm-dd
         OUTPUT yyyy-mm-dd -->
    <xsl:message>this-date(date : <xsl:value-of select="normalize-space(/date)"
      />)</xsl:message>
  ...
  <xsl:template match="date">
    ...
    <xsl:message>date(year : <xsl:value-of select="/year"/>, day : <xsl:value-of
        select="@day"/>, month : <xsl:value-of select="@month"/>, before 01/1 : <xsl:value-of
        select="@year-1"/>)</xsl:message>
    <xsl:choose>
      <xsl:when test="@*">
        <date>
          <xsl:choose>
  ...
  <xsl:template match="weekday-after">
    <!-- INPUT @day : weekday string, e.g. "Sunday"
         OUTPUT yyyy-mm-dd -->
    <xsl:message>weekday-after(day : <xsl:value-of select="@day"/>, date to be
  ...
  <xsl:template match="weekday-before">
    <!-- INPUT @day : weekday string, e.g. "Sunday"
         OUTPUT yyyy-mm-dd -->
    <xsl:message>weekday-before(day : <xsl:value-of select="@day"/>, date to be
      evaluated)</xsl:message>
  ...
  <xsl:template match="weekday-before-or-self">
    <!-- INPUT @day : weekday string, e.g. "Sunday"
         OUTPUT yyyy-mm-dd -->
    <xsl:message>weekday-before-or-self(day : <xsl:value-of select="@day"/>, date to be
      evaluated)</xsl:message>
    <xsl:variable name="daterules">
      <weekday-after day="{@day}">
        <days-before nr="7">
  ...
  <xsl:template match="relative-to-next-years">
    ...
    <xsl:call-template name="cache">
      <xsl:with-param name="mode" select="'c2d'" tunnel="yes"/>
      <xsl:with-param name="year" select="/year + 1" tunnel="yes"/>
      <xsl:with-param name="coordinates" select="$coordinates" tunnel="yes"/>
    </xsl:call-template>
  ...
  <xsl:template match="transfer">
    ...
    <xsl:message>transfer(sets : <xsl:value-of select="@sets"/>, rank : <xsl:value-of select="@rank"
      />)</xsl:message>
    ...
    <xsl:variable name="coordinates">
      <xsl:for-each select="//coordinaterules[matches($sets,@set)]">
        <xsl:variable name="rankprecedence">
          ...
          <xsl:message>rank : <xsl:value-of select="number($liturgicalday/rank/@nr)"/>; precedence :
              <xsl:value-of select="number($liturgicalday/precedence)"/></xsl:message>
          <xsl:choose>
            <xsl:when test="$liturgicalday/precedence">
              <xsl:value-of
                select="100 * number($liturgicalday/rank/@nr) + number($liturgicalday/precedence)"/>
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:variable name="ruleset">
          <!-- to avoid endless looping, the set-coordinates template should only return coordinates for days with minimum rank and precedence
            ...
            <adhocruleset>
              <xsl:message>transfer - finding overlap with minrankprecedence <xsl:value-of
                  select="$rankprecedence"/></xsl:message>
    ...
    <xsl:variable name="overlapping-liturgicaldays"
      select="//liturgicalday[matches($coordinates,coordinates)]"/>
    <xsl:message>overlapping-liturgicaldays: <xsl:copy-of select="$overlapping-liturgicaldays"
      /></xsl:message>
    <xsl:variable name="overlapping-liturgicaldays-of-higher-rank"
      select="$overlapping-liturgicaldays[number(rank/@nr) &lt; number($rank)]"/>
    <xsl:message>overlapping-liturgicaldays-of-higher-rank: <xsl:copy-of
        select="$overlapping-liturgicaldays-of-higher-rank"/></xsl:message>
    <xsl:choose>
      <xsl:when test="$overlapping-liturgicaldays-of-higher-rank">
        <xsl:message>TRANSFERRING from <xsl:value-of select="$date"/> for <xsl:value-of
            select="$overlapping-liturgicaldays/coordinates"/></xsl:message>
        <xsl:variable name="ruleset">
  ...
  <xsl:template match="coordinates">
    ...
    <xsl:message>coordinates(year : <xsl:value-of select="/year"/>, set :
        <xsl:value-of select="@set"/>, day : <xsl:value-of select="@day"/>, month : <xsl:value-of
        select="@month"/>, before 01/1 : <xsl:value-of select="@year-1"/>)</xsl:message>
    <xsl:variable name="date">
      <xsl:choose>
        <xsl:when test="@year-1">
          <xsl:value-of
            select="xs:date(concat(/year - 1,'-01-01')) + xs:yearMonthDuration(concat('P',@month - 1,'M')) + xs:dayTimeDuration(concat('P',@day - 1,'D'))"
          />
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of
            select="xs:date(concat(/year,'-01-01')) + xs:yearMonthDuration(concat('P',@month - 1,'M')) + xs:dayTimeDuration(concat('P',@day - 1,'D'))"
          />
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
  ...
  <xsl:template match="set-coordinates">
    ...
    <xsl:message>set-coordinates (set : <xsl:value-of select="@set"/>; minrankprecedence :
        <xsl:value-of select="@minrankprecedence"/>; date is to be evaluated)</xsl:message>
    <xsl:variable name="date">
      <xsl:apply-templates/>
    </xsl:variable>
    <xsl:call-template name="cache">
  ...
  <xsl:template match="query-set">
    <!-- INPUT $date : yyyy-mm-dd
       @set : name of a set of liturgical days
       OUTPUT for each liturgical day in @set, that has higher rankprecedence than /minrankprecedence, the daterules are applied and
    ...
    <xsl:message>query-set(date : <xsl:value-of select="normalize-space(/date)"/>,
      set : <xsl:value-of select="@set"/>)</xsl:message>
    <xsl:for-each select="//liturgicalday[set = current()/@set]">
      ...
      <xsl:message>querying <xsl:value-of select="set"/>; <xsl:value-of
          select="number(/minrankprecedence)"/> = 0 or &gt; <xsl:value-of
          select="number($rankprecedence)"/></xsl:message>
      <xsl:if
        test="number(/minrankprecedence) = 0 or number(/minrankprecedence) &gt; number($rankprecedence)">
        <xsl:variable name="candidate">
          <xsl:apply-templates select="daterules"/>
        </xsl:variable>
        <xsl:if
          test="not(normalize-space($candidate) = '') and (normalize-space($candidate) = /date or @anydate)">
  ...
  <xsl:template match="query-coordinates">
    <!-- INPUT $date : yyyy-mm-dd
       ...
       OUTPUT for the liturgical day(s) in @set that match @coordinates, and that have higher rankprecedence than /minrankprecedence, 
       the daterules are applied and if the date matches $date, 
       the <coordinates> for that liturgical day are returned.
       If anydate is specified all matching coordinates are returned concatenated. -->
    <xsl:message>query-coordinates(date : <xsl:value-of
        select="normalize-space(/date)"/>, set : <xsl:value-of select="@set"/>,
      coordinates to be evaluated)</xsl:message>
    <xsl:variable name="coordinates">
      <xsl:apply-templates/>
    </xsl:variable>
    <xsl:for-each select="//liturgicalday[set=current()/@set and coordinates=$coordinates]">
      <xsl:message>querying <xsl:value-of select="coordinates"/> in <xsl:value-of select="set"
        /></xsl:message>
      <xsl:if
        test="number(/minrankprecedence) = 0 or number(/minrankprecedence) &gt; 100 * number(rank/@nr) + number(precedence)">
        <xsl:variable name="candidate">
          <xsl:apply-templates select="daterules"/>
        </xsl:variable>
        <xsl:message>testing if <xsl:value-of select="$candidate"/> equals <xsl:value-of
            select="/date"/></xsl:message>
        <xsl:if
          test="not(normalize-space($candidate) = '') and ($candidate = /date or @anydate)">
          <xsl:value-of select="coordinates"/>
        </xsl:if>
      </xsl:if>
    </xsl:for-each>
  ...
  <xsl:template match="test-day">
    ...
    <xsl:message>test-day(day to be evaluated, date : <xsl:value-of
        select="normalize-space(/date)"/>)</xsl:message>
    <xsl:variable name="date">
      <xsl:apply-templates/>
    </xsl:variable>
  ...
  <xsl:template match="not-after">
    ...
    <xsl:if test="xs:date($date1) &lt;= xs:date($date2)">true</xsl:if>
    <xsl:message>/not-after(date1 : <xsl:value-of select="$date1"/>, date2 : <xsl:value-of
        select="$date2"/>)</xsl:message>
  </xsl:template>
  <xsl:template match="matches">
    ...
    <xsl:message>/matches(string1 : <xsl:value-of select="$string1"/>, string2 : <xsl:value-of
        select="$string2"/>)</xsl:message>
  </xsl:template>
  <xsl:template match="equals">
    ...
    <xsl:message>/equals(string1 : <xsl:value-of select="$string1"/>, string2 : <xsl:value-of
        select="$string2"/>)</xsl:message>
  </xsl:template>
  ...
  <xsl:template match="format">
    ...
    <xsl:message>format(template : <xsl:value-of select="@template"/>, number to be
      evaluated)</xsl:message>
    <xsl:variable name="number">
      <xsl:apply-templates/>
    </xsl:variable>
    <xsl:value-of select="format-number($number,@template)"/>
    <xsl:message>/format(template : <xsl:value-of select="@template"/>, number : <xsl:value-of
  ...
  <xsl:template match="mmdd">
    ...
    <xsl:value-of
      select="concat(format-number(fn:month-from-date(xs:date($date)),'00'),format-number(fn:day-from-date(xs:date($date)),'00'))"/>
